    ...
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            do
                ...
                texturePack:newDropdown("world/texture_pack/pack", false, {
                    "Minecraft",
                    "Grods"
                }):set("Minecraft")
            end
            ...
            -- Hit Sound
            local hitSound = world:newSection("left", "Hit Sound")
            do
                hitSound:newToggle("world/sound/hit_enabled"):setLabel("Hit Sound")
                hitSound:newDropdown("world/sound/hit_sound", false, {
                    "선택하기",
                    "sparkle",
                    "bubble",
                    "rust",
                ...
                hitSound:newSlider("world/sound/hit_volume", 0, 2, 0.1):set(1):setLabel("Sound Volume")
            end

            -- Kill Sound
            local killSound = world:newSection("right", "Kill Sound")
            do
                killSound:newToggle("world/sound/kill_enabled"):setLabel("Kill Sound")
                killSound:newDropdown("world/sound/kill_sound", false, {
                    "선택하기",
                    "Default",
                    "Bell",
                    "Bubble",
        ...
        -- ====================== HIT SOUNDS ======================
        -- Plays a separate sound only when another humanoid actually loses health.
        -- Do NOT replace existing SoundId values: doing so makes footsteps, UI sounds,
        -- weapon sounds, etc. play the selected hit sound.
        ...
        local hitSoundAssetCache = {}
        local hitSoundConnections = {}

        local function getHitSoundAsset(url)
            local success, result = pcall(function()
                if writefile and isfile and getcustomasset then
                    local fileName = "hit_" .. tostring(#url % 100000) .. ".asset"
        ...
        local function getSelectedHitSoundAsset()
            local selected = hitSoundDropdown and hitSoundDropdown.value or "선택하기"
            local url = HIT_SOUNDS[selected]
            if not url then
                return nil
            end
            if not hitSoundAssetCache[selected] then
                hitSoundAssetCache[selected] = getHitSoundAsset(url)
            end
            return hitSoundAssetCache[selected]
        end

        local function playHitSound()
            if not hitSoundToggle or not hitSoundToggle.value then
                return
            end

            local asset = getSelectedHitSoundAsset()
            if not asset then
                return
            end
            ...
            sound.Name = "MinhoHubHitSound"
            sound.SoundId = asset
            sound.Volume = math.clamp(tonumber(hitSoundVolume and hitSoundVolume.value) or 1, 0, 2)
        ...
        local function disconnectHitSoundConnections()
            for humanoid, connection in pairs(hitSoundConnections) do
                pcall(function() connection:Disconnect() end)
                hitSoundConnections[humanoid] = nil
            end
        end

        local function watchHumanoid(humanoid)
            if not humanoid or not humanoid:IsA("Humanoid") or hitSoundConnections[humanoid] then
                return
            end

            local lastHealth = humanoid.Health
            hitSoundConnections[humanoid] = humanoid.HealthChanged:Connect(function(newHealth)
                if newHealth < lastHealth and humanoid.Parent then
        ...
        local function refreshHitSoundWatchers()
            disconnectHitSoundConnections()
            if not hitSoundToggle or not hitSoundToggle.value or not HIT_SOUNDS[hitSoundDropdown.value] then
                return
            end
        ...
        if hitSoundToggle then
            hitSoundToggle.changed:Connect(function()
                refreshHitSoundWatchers()
            end)
        end

        if hitSoundDropdown then
            hitSoundDropdown.changed:Connect(function()
                refreshHitSoundWatchers()
            end)
        end
        ...
        game.DescendantAdded:Connect(function(obj)
            if obj:IsA("Humanoid") then
                task.defer(function()
                    if hitSoundToggle and hitSoundToggle.value and HIT_SOUNDS[hitSoundDropdown.value] then
                        watchHumanoid(obj)
                    end
                end)
            end
        ...
        task.delay(1.5, refreshHitSoundWatchers)
