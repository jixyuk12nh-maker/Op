...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        -- ====================== HIT SOUNDS ======================
        -- Replaces SoundId values using the selected remote sound asset.
        -- The UI toggle controls whether the replacement is active.
        ...
        local function getSelectedHitSoundAsset()
            ...
            return hitSoundAssetCache[selected], selected
        end

        local function applyHitSound(obj)
            if not hitSoundToggle or not hitSoundToggle.value or not obj or not obj:IsA("Sound") then
                return
            end

            local asset, selected = getSelectedHitSoundAsset()
            if not asset then
                return
            end

            if hitSoundOriginals[obj] == nil then
                hitSoundOriginals[obj] = obj.SoundId
            end
            ...
            if selected then
                print("Hit Sound loaded: " .. tostring(selected))
            end
        end

        local function restoreHitSounds()
            for obj, original in pairs(hitSoundOriginals) do
                if obj and obj.Parent and obj:IsA("Sound") then
                    pcall(function()
                        obj.SoundId = original
                    end)
                end
            end
            table.clear(hitSoundOriginals)
        end

        local function refreshHitSounds()
            if not hitSoundToggle or not hitSoundToggle.value then
                restoreHitSounds()
                return
            end

            if not HIT_SOUNDS[hitSoundDropdown.value] then
                return
            end

            for _, descendant in ipairs(game:GetDescendants()) do
                task.spawn(applyHitSound, descendant)
            end
        end
