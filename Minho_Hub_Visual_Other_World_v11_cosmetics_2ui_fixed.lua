...
-- ============================================================
-- CosmeticChanger engine (from user-provided ㅅㅋㅊㅇㅈ.txt)
-- ============================================================
-- ============================================================
...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            -- Cosmetics: opens a second UI with the same UIBase/theme.
            local cosmetics = misc:newSection("left", "Cosmetics")
            do
                local cosmeticsToggle = cosmetics:newToggle("misc/cosmetics/show_changer"):setLabel("Cosmetics")

                local cosmeticUI = UIBase.new():setLabel("Minho Hub - Cosmetics")
                -- Do not assign a second keybind. The main Cosmetics toggle controls this UI.
                local cosmeticTabs = cosmeticUI:newTabList()
                local changerTab = cosmeticTabs:newTab("Cosmetics"):intoSections()

                local weaponSection = changerTab:newSection("left", "Weapon")
                local categorySection = changerTab:newSection("left", "Category")
                local cosmeticSection = changerTab:newSection("right", "Cosmetic")
                local statusSection = changerTab:newSection("right", "Status")

                local function getAPI()
                    local api = rawget(shared, "CosmeticChanger")
                ...
                local function getWeapons()
                    local api = getAPI()
                    if not api or type(api.GetWeapons) ~= "function" then return {} end
                    local ok, result = pcall(function() return api.GetWeapons() end)
                    if ok and type(result) == "table" then return result end
                    return {}
                end

                local function getCosmetics(weapon, category)
                    local api = getAPI()
                    if not api or type(api.GetCosmetics) ~= "function" then return {} end
                    if type(weapon) ~= "string" or weapon == "" then return {} end
                    local ok, result = pcall(function() return api.GetCosmetics(weapon, category) end)
                    if ok and type(result) == "table" then return result end
                    return {}
                end

                local weapons = getWeapons()
                if #weapons == 0 then weapons = {"No weapons found"} end

                local weaponDrop = weaponSection:newDropdown("cosmetics/weapon", false, weapons):set(weapons[1])
                local categoryDrop = categorySection:newDropdown("cosmetics/category", false, {"Skin", "Wrap", "Charm", "Finisher"}):set("Skin")
                local cosmeticDrop = cosmeticSection:newDropdown("cosmetics/cosmetic", false, {"No cosmetics found"}):set("No cosmetics found")
                local statusLabel = statusSection:newLabel("cosmetics/status"):setLabel("Ready")

                local function currentValue(feature)
                    local value = feature and feature.value
                    if type(value) == "table" then value = value[1] end
                    return value
                end

                local function replaceDropdown(drop, options)
                    options = (#options > 0) and options or {"No cosmetics found"}
                    -- Iterate over a copy because remove() mutates drop.options.
                    local old = {}
                    for _, option in ipairs(drop.options or {}) do old[#old + 1] = option end
                    for _, option in ipairs(old) do pcall(function() drop:remove(option) end) end
                    for _, option in ipairs(options) do pcall(function() drop:add(option) end) end
                    pcall(function() drop:set(options[1]) end)
                end

                local refreshing = false
                local function refreshCosmeticList()
                    if refreshing then return end
                    refreshing = true

                    local weapon = currentValue(weaponDrop)
                    local category = currentValue(categoryDrop)
                    if weapon == "No weapons found" or type(weapon) ~= "string" then
                        replaceDropdown(cosmeticDrop, {"No cosmetics found"})
                        statusLabel:setLabel("Cosmetic API unavailable")
                        refreshing = false
                        return
                    end

                    local list = getCosmetics(weapon, category or "Skin")
                    if #list == 0 then
                        replaceDropdown(cosmeticDrop, {"No cosmetics found"})
                        statusLabel:setLabel("No cosmetics found")
                    else
                        replaceDropdown(cosmeticDrop, list)
                        statusLabel:setLabel("Ready - " .. tostring(#list) .. " cosmetics")
                    end
                    refreshing = false
                end

                weaponDrop.changed:Connect(refreshCosmeticList)
                categoryDrop.changed:Connect(refreshCosmeticList)

                cosmeticDrop.changed:Connect(function(value)
                    local api = getAPI()
                    if not api or type(api.Apply) ~= "function" then return end
                    local weapon = currentValue(weaponDrop)
                    local category = currentValue(categoryDrop)
                    local cosmetic = value
                    if type(cosmetic) == "table" then cosmetic = cosmetic[1] end
                    if type(weapon) ~= "string" or weapon == "No weapons found" then return end
                    if type(cosmetic) ~= "string" or cosmetic == "No cosmetics found" then return end

                    local ok = pcall(function()
                        api.Apply(weapon, cosmetic, category or "Skin")
                    end)
                    statusLabel:setLabel(ok and ("Applied - " .. cosmetic) or "Apply failed")
                end)

                -- Restore saved equipment only if the API exposes both functions.
                task.defer(function()
                    local api = getAPI()
                    if api and type(api.GetEquipData) == "function" and type(api.LoadEquipData) == "function" then
                        local ok, data = pcall(function() return api.GetEquipData() end)
                        if ok then pcall(function() api.LoadEquipData(data) end) end
                    end
                    refreshCosmeticList()
                end)

                cosmeticUI:setVisible(false)
                cosmeticsToggle.changed:Connect(function(state)
                    cosmeticUI:setVisible(state == true)
                end)

                local unloadFeature = base.features["settings/menu/unload"]
                if unloadFeature then
                    unloadFeature.changed:Connect(function()
                        if unloadFeature.value == true then
                            pcall(function() cosmeticUI:Destroy() end)
                        end
                    end)
                end
            end
