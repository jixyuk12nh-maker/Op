...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        -- ====================== HIT SOUNDS ======================
        -- Only play the selected hit sound when the game creates/plays
        -- a damage-related Sound. Normal game sounds are left untouched.
        ...
        local function getHitSoundAsset(name)
            local url = HIT_SOUNDS[name]
            if not url then
                return nil
            end

            if hitSoundCache[name] then
                return hitSoundCache[name]
            end

            local success, asset = pcall(function()
                if writefile and isfile and getcustomasset then
                    local safeName = name:gsub("[^%w_]", "_")
                    local fileName = "MinhoHub_Hit_" .. safeName .. ".mp3"

                    if not isfile(fileName) then
                        local data = game:HttpGet(url)
                        if not data or #data <= 80 then
                            error("Hit sound download failed")
                        end
        ...
        local function playHitSound()
            ...
            local asset = getHitSoundAsset(selected)
            if not asset then
                return
            end

            local sound = Instance.new("Sound")
            sound.Name = "MinhoHub_HitSound"
            sound.SoundId = asset
            sound.Volume = 1
        ...
        -- Detect actual damage/hit events without replacing every SoundId.
        local function isHitLikeSound(obj)
            if not obj or not obj:IsA("Sound") then
                return false
            end
            ...
            local keywords = {
                "hit", "damage", "hurt", "impact", "impacthit",
                "shoot", "bullet", "slash", "melee", "blood"
            }

            for _, keyword in ipairs(keywords) do
                if string.find(name, keyword, 1, true)
                    or string.find(parentName, keyword, 1, true) then
                    return true
                end
            end
        ...
        local function hookHitSound(obj)
            if not obj or not obj:IsA("Sound") then
                return
            end
            if hitSoundConnections[obj] then
                return
            end

            local connection = obj.Played:Connect(function()
                if hitSoundToggle and hitSoundToggle.value and isHitLikeSound(obj) then
                    playHitSound()
                end
            end)

            hitSoundConnections[obj] = connection
        end
        ...
        game.DescendantAdded:Connect(function(obj)
            task.defer(hookHitSound, obj)
        end)

        if hitSoundToggle and hitSoundToggle.changed then
            hitSoundToggle.changed:Connect(function(enabled)
                if enabled then
                    -- Preload selected asset when the user turns it on.
                    task.spawn(function()
                        getHitSoundAsset(hitSoundDropdown and hitSoundDropdown.value)
                    end)
                end
            end)
        end

        if hitSoundDropdown and hitSoundDropdown.changed then
            hitSoundDropdown.changed:Connect(function(selected)
                if hitSoundToggle and hitSoundToggle.value then
                    task.spawn(function()
                        getHitSoundAsset(selected)
                    end)
                end
            end)
        end
