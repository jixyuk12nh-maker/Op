...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            -- Cosmetics opens a SECOND UI using the exact same UIBase/theme.
            ...
            do
                ...
                cosmeticUI:setKeybind(Enum.KeyCode.RightShift)
                ...
                local function pcGetWeapons()
                    local api = shared and shared.CosmeticChanger
                    if not api or type(api.GetWeapons) ~= "function" then return {} end
                    local ok, result = pcall(api.GetWeapons)
                    return ok and type(result) == "table" and result or {}
                end

                local function pcGetCosmetics(weapon, category)
                    local api = shared and shared.CosmeticChanger
                    if not api or type(api.GetCosmetics) ~= "function" or not weapon then return {} end
                    local ok, result = pcall(api.GetCosmetics, weapon, category)
                    return ok and type(result) == "table" and result or {}
                end

                local weaponOptions = pcGetWeapons()
                if #weaponOptions == 0 then weaponOptions = {"No weapons found"} end
                local weaponDrop = weaponSection:newDropdown("cosmetics/weapon", false, weaponOptions):set(weaponOptions[1])
                ...
                statusSection:newLabel("cosmetics/status"):setLabel("Ready")

                local function clearDropdown(drop, options, value)
                    for _, old in ipairs(drop.options or {}) do
                        pcall(function() drop:remove(old) end)
                    end
                    for _, option in ipairs(options) do
                        pcall(function() drop:add(option) end)
                    end
                    if #options > 0 then
                        pcall(function() drop:set(value or options[1]) end)
                    end
                end

                local function refreshCosmetics()
                    local weapon = weaponDrop.value
                    local category = categoryDrop.value
                    if type(weapon) == "table" then weapon = weapon[1] end
                    if type(category) == "table" then category = category[1] end
                    if weapon == "No weapons found" then
                        clearDropdown(cosmeticDrop, {"No cosmetics found"}, "No cosmetics found")
                        return
                    end
                    local list = pcGetCosmetics(weapon, category)
                    if #list == 0 then list = {"No cosmetics found"} end
                    clearDropdown(cosmeticDrop, list, list[1])
                    pcall(function() statusSection.features = statusSection.features end)
                end

                weaponDrop.changed:Connect(refreshCosmetics)
                categoryDrop.changed:Connect(refreshCosmetics)
                cosmeticDrop.changed:Connect(function(value)
                    ...
                    if not weapon or weapon == "No weapons found" or not cosmetic or cosmetic == "No cosmetics found" then return end
                    pcall(api.Apply, weapon, cosmetic, category)
                end)

                local saved = shared and shared.CosmeticChanger and shared.CosmeticChanger.GetEquipData
                if type(saved) == "function" then
                    local ok, data = pcall(saved)
                    if ok and shared.CosmeticChanger.LoadEquipData then
                        pcall(shared.CosmeticChanger.LoadEquipData, data)
                    end
                end
                refreshCosmetics()
