...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            do
                local loadout = aimbot:newSection("left", "Loadout")

                local StarterPlayer = game:GetService("StarterPlayer")
                local weaponsFolder = StarterPlayer:FindFirstChild("StarterPlayerScripts")
                weaponsFolder = weaponsFolder and weaponsFolder:FindFirstChild("Assets")
                weaponsFolder = weaponsFolder and weaponsFolder:FindFirstChild("ViewModels")
                weaponsFolder = weaponsFolder and weaponsFolder:FindFirstChild("Weapons")

                local weaponList = {}
                local function addWeapons(folder)
                ...
                addWeapons(weaponsFolder)
                addWeapons(weaponsFolder and weaponsFolder:FindFirstChild("Unobtainable"))

                table.sort(weaponList)

                local function getDefault(preferred)
                    if table.find(weaponList, preferred) then
                        return preferred
                    end
                    return weaponList[1] or preferred
                end

                local autoLoad = loadout:newToggle("main/loadout/auto_load"):setLabel("Auto Load")
                loadout:newToggle("main/loadout/silent_mode"):setLabel("Silent Mode")

                loadout:newLabel("main/loadout/primary_label"):setLabel("Primary")
                local primary = loadout:newDropdown("main/loadout/primary", false, weaponList):set(getDefault("Assault Rifle"))

                loadout:newLabel("main/loadout/secondary_label"):setLabel("Secondary")
                local secondary = loadout:newDropdown("main/loadout/secondary", false, weaponList):set(getDefault("Handgun"))

                loadout:newLabel("main/loadout/melee_label"):setLabel("Melee")
                local melee = loadout:newDropdown("main/loadout/melee", false, weaponList):set(getDefault("Fists"))

                loadout:newLabel("main/loadout/utility_label"):setLabel("Utility")
                local utility = loadout:newDropdown("main/loadout/utility", false, weaponList):set(getDefault("Grenade"))

                local loadoutLoopRunning = false
                local function pickLoadout()
                    local remote = ReplicatedStorage.Remotes.Replication.Fighter.PickWeapons
                    remote:FireServer({
                        primary.value,
                        secondary.value,
                        melee.value,
                ...
                local function runLoadoutLoop()
                    if loadoutLoopRunning or not autoLoad.value then return end
                    loadoutLoopRunning = true
                    task.spawn(function()
                        while autoLoad.value do
                            pcall(pickLoadout)
                            task.wait(0.5)
                        end
                ...
                autoLoad.changed:Connect(function()
            ...
            do
                -- All requested Aimbot controls are placed on the right.
                local settings = aimbot:newSection("right", "Aimbot")
                ...
                settings:newToggle("main/aimbot/show_fov"):setLabel("Show Fov")
                settings:newToggle("main/aimbot/outline"):setLabel("Outline")
                settings:newToggle("main/aimbot/fill"):setLabel("Fill")
                settings:newToggle("main/aimbot/lerp"):setLabel("Lerp")
                settings:newToggle("main/aimbot/moving_rotation"):setLabel("Moving Rotation")
                settings:newToggle("main/aimbot/speed"):setLabel("Speed")
                settings:newToggle("main/aimbot/fov_settings"):setLabel("Fov Settings")
                settings:newToggle("main/aimbot/radius"):setLabel("Radius")
                settings:newToggle("main/aimbot/smoothing"):setLabel("Smoothing")
            end

            local silentaimtab = mainTabs:newTab("Silent Aim"):intoSections()
            do
                -- All requested Silent Aim controls are placed on the right.
                local settings = silentaimtab:newSection("right", "Silent Aim")
                ...
                settings:newToggle("main/silent_aim/show_fov"):setLabel("Show Fov")
                settings:newToggle("main/silent_aim/outline"):setLabel("Outline")
                settings:newToggle("main/silent_aim/fill"):setLabel("Fill")
                settings:newToggle("main/silent_aim/lerp"):setLabel("Lerp")
                settings:newToggle("main/silent_aim/moving_rotation"):setLabel("Moving Rotation")
                settings:newToggle("main/silent_aim/speed"):setLabel("Speed")
                settings:newToggle("main/silent_aim/fov_settings"):setLabel("Fov Settings")
                settings:newToggle("main/silent_aim/position_on_target"):setLabel("Position On Target")
                settings:newToggle("main/silent_aim/position_on_barrel"):setLabel("Position On Barrel")
                settings:newToggle("main/silent_aim/radius"):setLabel("Radius")
            end
        ...
        do
            ...
            do
                ...
                info:newLabel("world/main/label_info2"):setLabel("skin in lobby and we will apply it in game")
        ...
        do
            ...
            do
                ...
                miscTools:newToggle("misc/utilities/device_spoofer"):setLabel("Device Spoofer")
                miscTools:newDropdown("misc/utilities/device_spoofer_mode", false, {
                    "Touch",
                    "Gamepad",
                    "MouseKeyboard",
                    "VR",
    ...
    -- Device Spoofer
    do
        local deviceSpoofer = base.features["misc/utilities/device_spoofer"]
        local deviceMode = base.features["misc/utilities/device_spoofer_mode"]
        ...
        local function setDevice(mode)
            pcall(function()
                ...
                if setControls then
                    setControls:FireServer(mode)
                end
            end)
        end

        if deviceSpoofer and deviceMode then
            deviceSpoofer.changed:Connect(function(enabled)
                if enabled then
                    setDevice(deviceMode.value)
                else
                    setDevice(getCurrentDevice())
                end
            end)

            deviceMode.changed:Connect(function(mode)
                if deviceSpoofer.value then
                    setDevice(mode)
                end
            end)
