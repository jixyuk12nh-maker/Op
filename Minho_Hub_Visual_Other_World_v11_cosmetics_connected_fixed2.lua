...
if not (elib and clib and ilib and consts and dctrl and clib.Cosmetics) then
    warn("[CosmeticChanger] 필수 모듈 로드 실패 - 기존 shared.CosmeticChanger가 있으면 사용합니다.")
    if shared and shared.CosmeticChanger then
        CosmeticChanger = shared.CosmeticChanger
    else
        CosmeticChanger = nil
    end
else
...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            do
                local openButton = cosmetics:newButton("misc/cosmetics/open"):setLabel("Cosmetics")
                local win, list, weapons, cats, status
                local selectedWeapon, selectedType = nil, "Skin"
                local categories = {{"Skin","Skins"},{"Wrap","Wraps"},{"Charm","Charm"},{"Finisher","Finisher"}}

                local function button(parent, text, size)
                    local b = Instance.new("TextButton")
                    b.Size = size or UDim2.new(1,-8,0,22)
                ...
                local function refresh()
                    if not list or not selectedWeapon then return end
                    clear(list)
                    local api = shared and shared.CosmeticChanger
                    if not api or type(api.GetCosmetics) ~= "function" then status.Text = "Cosmetic API unavailable"; return end
                    local ok, data = pcall(api.GetCosmetics, selectedWeapon, selectedType)
                    if not ok or type(data) ~= "table" then status.Text = "Failed to load cosmetics"; return end
                    status.Text = selectedWeapon .. " / " .. selectedType .. " / " .. #data
                    if #data == 0 then
                        local b=button(list,"No cosmetics found"); b.TextXAlignment=Enum.TextXAlignment.Center; return
                    end
                    for _,name in ipairs(data) do
                        local b=button(list,name)
                        b.MouseButton1Click:Connect(function()
                            local api = shared and shared.CosmeticChanger
                            if not api or type(api.Apply) ~= "function" then status.Text="Cosmetic API unavailable"; return end
                            local okApply,err=pcall(api.Apply,selectedWeapon,name,selectedType)
                            status.Text=okApply and ("Equipped: "..name) or ("Apply failed: "..tostring(err))
                        end)
                    end
                end
                ...
                local function selectWeapon(name)
                    ...
                    for _,v in ipairs(weapons:GetChildren()) do
                        if v:IsA("TextButton") then v.TextColor3=(v.Text==name) and Color3.fromRGB(55,175,225) or Color3.fromRGB(255,255,255) end
                    end
                    refresh()
                end

                local function build()
                    if win then win.Visible=true; return end
                    win=Instance.new("Frame")
                    win.Name="CosmeticsWindow"; win.Size=UDim2.new(0,500,0,330); win.Position=UDim2.new(.5,-250,.5,-165)
                    win.BackgroundColor3=Color3.fromRGB(55,175,225); win.BorderColor3=Color3.fromRGB(0,0,0); win.ZIndex=50; win.Parent=base.instances.gui
                    local bg=Instance.new("Frame"); bg.Position=UDim2.new(0,1,0,1); bg.Size=UDim2.new(1,-2,1,-2); bg.BackgroundColor3=Color3.fromRGB(30,30,30); bg.BorderSizePixel=0; bg.ZIndex=50; bg.Parent=win
                    local title=Instance.new("TextLabel"); title.BackgroundTransparency=1; title.Font=Enum.Font.Arial; title.TextSize=12; title.TextStrokeTransparency=0; title.TextColor3=Color3.fromRGB(255,255,255); title.Text="Cosmetics"; title.TextXAlignment=Enum.TextXAlignment.Left; title.Position=UDim2.new(0,6,0,5); title.Size=UDim2.new(1,-35,0,15); title.ZIndex=51; title.Parent=bg
                    local close=button(bg,"X",UDim2.new(0,24,0,18)); close.Position=UDim2.new(1,-28,0,3); close.TextXAlignment=Enum.TextXAlignment.Center; close.ZIndex=51; close.MouseButton1Click:Connect(function() win.Visible=false end)
                    local content=Instance.new("Frame"); content.Position=UDim2.new(0,5,0,23); content.Size=UDim2.new(1,-10,1,-28); content.BackgroundColor3=Color3.fromRGB(0,0,0); content.BorderColor3=Color3.fromRGB(50,50,50); content.ZIndex=50; content.Parent=bg
                    local left=Instance.new("Frame"); left.Position=UDim2.new(0,1,0,1); left.Size=UDim2.new(0,145,1,-2); left.BackgroundColor3=Color3.fromRGB(25,25,25); left.BorderSizePixel=0; left.ZIndex=50; left.Parent=content
                    local right=Instance.new("Frame"); right.Position=UDim2.new(0,148,0,1); right.Size=UDim2.new(1,-149,1,-2); right.BackgroundColor3=Color3.fromRGB(25,25,25); right.BorderSizePixel=0; right.ZIndex=50; right.Parent=content
                    local wt=Instance.new("TextLabel"); wt.BackgroundTransparency=1; wt.Font=Enum.Font.Arial; wt.TextSize=12; wt.TextStrokeTransparency=0; wt.TextColor3=Color3.fromRGB(255,255,255); wt.Text="Weapons"; wt.TextXAlignment=Enum.TextXAlignment.Left; wt.Position=UDim2.new(0,5,0,5); wt.Size=UDim2.new(1,-10,0,15); wt.ZIndex=51; wt.Parent=left
                    weapons=Instance.new("ScrollingFrame"); weapons.Position=UDim2.new(0,4,0,25); weapons.Size=UDim2.new(1,-8,1,-30); weapons.BackgroundTransparency=1; weapons.BorderSizePixel=0; weapons.ScrollBarThickness=1; weapons.AutomaticCanvasSize=Enum.AutomaticSize.Y; weapons.ZIndex=51; weapons.Parent=left
                    local wl=Instance.new("UIListLayout"); wl.Padding=UDim.new(0,4); wl.Parent=weapons
                    cats=Instance.new("Frame"); cats.Position=UDim2.new(0,5,0,4); cats.Size=UDim2.new(1,-10,0,22); cats.BackgroundTransparency=1; cats.ZIndex=51; cats.Parent=right
                    local cl=Instance.new("UIListLayout"); cl.FillDirection=Enum.FillDirection.Horizontal; cl.Padding=UDim.new(0,4); cl.Parent=cats
                    for _,cat in ipairs(categories) do
                        local c=button(cats,cat[2],UDim2.new(0,78,0,20)); c.TextXAlignment=Enum.TextXAlignment.Center; c.MouseButton1Click:Connect(function() selectedType=cat[1]; for _,x in ipairs(cats:GetChildren()) do if x:IsA("TextButton") then x.TextColor3=(x==c) and Color3.fromRGB(55,175,225) or Color3.fromRGB(255,255,255) end end; refresh() end)
                    end
                    status=Instance.new("TextLabel"); status.BackgroundTransparency=1; status.Font=Enum.Font.Arial; status.TextSize=11; status.TextStrokeTransparency=0; status.TextColor3=Color3.fromRGB(170,170,170); status.Text="Select a weapon"; status.TextXAlignment=Enum.TextXAlignment.Left; status.Position=UDim2.new(0,6,0,29); status.Size=UDim2.new(1,-12,0,15); status.ZIndex=51; status.Parent=right
                    list=Instance.new("ScrollingFrame"); list.Position=UDim2.new(0,5,0,48); list.Size=UDim2.new(1,-10,1,-53); list.BackgroundTransparency=1; list.BorderSizePixel=0; list.ScrollBarThickness=1; list.AutomaticCanvasSize=Enum.AutomaticSize.Y; list.ZIndex=51; list.Parent=right
                    local ll=Instance.new("UIListLayout"); ll.Padding=UDim.new(0,4); ll.Parent=list
                    base:makeDraggable(title,windowTrove,function(input) win.Position=UDim2.new(0,input.Position.X-250,0,input.Position.Y-12) end)
                    local api = shared and shared.CosmeticChanger
                    local ok,data = api and type(api.GetWeapons) == "function" and pcall(api.GetWeapons) or false,nil
                    if ok and type(data)=="table" then
                        for _,name in ipairs(data) do local b=button(weapons,name); b.MouseButton1Click:Connect(function() selectWeapon(name) end) end
                        if data[1] then selectWeapon(data[1]) end
                    else status.Text="Failed to load weapons" end
                end

                openButton.changed:Connect(function() build(); win.Visible=true end)
            end
'''
