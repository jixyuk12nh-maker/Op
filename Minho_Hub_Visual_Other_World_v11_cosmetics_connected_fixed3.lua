    ...
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            do
                ...
                local cosmeticPanel = misc:newSection("right", "Skin Changer")
                local categoryDropdown = cosmeticPanel:newDropdown(
                    "misc/cosmetics/category", false,
                    {"Skins", "Wraps", "Charm", "Finisher"}
                ):set("Skins")

                local weaponNames = {}
                local weaponDropdown = cosmeticPanel:newDropdown(
                    "misc/cosmetics/weapon", false, {"No weapons found"}
                ):set("No weapons found")

                local cosmeticNames = {}
                local cosmeticDropdown = cosmeticPanel:newDropdown(
                    "misc/cosmetics/item", false, {"No cosmetics found"}
                ):set("No cosmetics found")

                cosmeticPanel:newLabel("misc/cosmetics/status"):setLabel("Ready")

                local changer = shared and shared.CosmeticChanger
                ...
                local function clearDropdown(dropdown)
                    for _, option in ipairs(table.clone(dropdown.options)) do
                        dropdown:remove(option)
                    end
                end

                local function getDropdownValue(dropdown, fallback)
                ...
                local function refreshCosmetics()
                    if not changer or type(changer.GetCosmetics) ~= "function" then
                        return
                    end

                    local weapon = getDropdownValue(weaponDropdown, nil)
                    if not weapon or weapon == "No weapons found" then
                        clearDropdown(cosmeticDropdown)
                        cosmeticDropdown:add("No cosmetics found")
                        cosmeticDropdown:set("No cosmetics found")
                        return
                    end

                    local ctype = categoryMap[getDropdownValue(categoryDropdown, "Skins")] or "Skin"
                    local ok, list = pcall(changer.GetCosmetics, weapon, ctype)
                    if not ok or type(list) ~= "table" then
                        list = {}
                    end

                    clearDropdown(cosmeticDropdown)
                    table.clear(cosmeticNames)
                    ...
                    for _, name in ipairs(list) do
                        ...
                        cosmeticDropdown:add(name)
                    end
                    cosmeticDropdown:set(cosmeticNames[1])
                end

                local function refreshWeapons()
                    if not changer or type(changer.GetWeapons) ~= "function" then
                        return
                    end

                    local ok, list = pcall(changer.GetWeapons)
                    if not ok or type(list) ~= "table" then
                        list = {}
                    end

                    clearDropdown(weaponDropdown)
                    table.clear(weaponNames)

                    for _, weapon in ipairs(list) do
                        weapon = tostring(weapon)
                        table.insert(weaponNames, weapon)
                        weaponDropdown:add(weapon)
                    end

                    if #weaponNames > 0 then
                        weaponDropdown:set(weaponNames[1])
                        refreshCosmetics()
                    else
                        weaponDropdown:add("No weapons found")
                        weaponDropdown:set("No weapons found")
                    end
                end

                if weaponDropdown.changed then
                    weaponDropdown.changed:Connect(refreshCosmetics)
                end

                if categoryDropdown.changed then
                    categoryDropdown.changed:Connect(refreshCosmetics)
                end

                if cosmeticDropdown.changed then
                    cosmeticDropdown.changed:Connect(function()
                        if not changer or type(changer.Apply) ~= "function" then return end
                        local weapon = getDropdownValue(weaponDropdown, nil)
                        local cosmetic = getDropdownValue(cosmeticDropdown, nil)
                        if not weapon or weapon == "No weapons found" or not cosmetic or cosmetic == "No cosmetics found" then return end
                        local ctype = categoryMap[getDropdownValue(categoryDropdown, "Skins")] or "Skin"
                        pcall(changer.Apply, weapon, cosmetic, ctype)
                    end)
                end

                if changer and type(changer.GetEquipData) == "function" and type(changer.LoadEquipData) == "function" then
                    local ok, saved = pcall(changer.GetEquipData)
                    if ok and saved then pcall(changer.LoadEquipData, saved) end
                end

                -- Keep the existing UI hidden until the Cosmetics toggle is enabled.
                if cosmeticPanel.instances and cosmeticPanel.instances.container then
                    cosmeticPanel.instances.container.Visible = false
                end

                if cosmeticsToggle.changed then
                    cosmeticsToggle.changed:Connect(function()
                        local enabled = cosmeticsToggle.value == true
                        if cosmeticPanel.instances and cosmeticPanel.instances.container then
                            cosmeticPanel.instances.container.Visible = enabled
                        end
                        if enabled then
                            refreshWeapons()
                        end
