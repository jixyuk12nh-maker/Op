...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        -- ====================== HIT SOUNDS ======================
        -- Safe runtime implementation:
        -- Nothing is downloaded or changed until Hit Sound is enabled
        -- and a real sound is selected.
        ...
        local function getHitSoundAsset(name)
            ...
            local success, asset = pcall(function()
                if writefile and isfile and getcustomasset then
                    local fileName = "MinhoHub_Hit_" .. name:gsub("[^%w_]", "_") .. ".mp3"
        ...
        local function restoreHitSounds()
            for sound, originalId in pairs(hitSoundOriginals) do
        ...
        local function applySelectedHitSound(sound)
            ...
            if hitSoundOriginals[sound] == nil then
                hitSoundOriginals[sound] = sound.SoundId
            end
        ...
        local function applyAllHitSounds()
            if not hitSoundToggle or not hitSoundToggle.value then
                restoreHitSounds()
                return
            end
            ...
            local selected = hitSoundDropdown and hitSoundDropdown.value
            if not HIT_SOUNDS[selected] then
                return
            end

            for _, obj in ipairs(game:GetDescendants()) do
                if obj:IsA("Sound") then
                    task.spawn(applySelectedHitSound, obj)
                end
            end
        end
        ...
        game.DescendantAdded:Connect(function(obj)
            if hitSoundToggle and hitSoundToggle.value then
                task.defer(applySelectedHitSound, obj)
            end
        end)
