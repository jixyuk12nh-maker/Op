...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        -- World Texture Pack
        -- Replaces the map texture asset 7658055825 with the selected pack texture.
        ...
        local function isWorldTextureTarget(obj)
            ...
            return string.match(value, "%d+") == WORLD_TEXTURE_ID
        end

        local function applyWorldTextureToObject(obj)
            if not worldTextureToggle or not worldTextureToggle.value then
                return
            end

            if not isWorldTextureTarget(obj) then
                return
            end
            ...
            if not success or type(current) ~= "string" then
                return
            end

            if worldTextureOriginals[obj] == nil then
                worldTextureOriginals[obj] = current
            end
            ...
            local url = WORLD_TEXTURES[tostring(selected)] or WORLD_TEXTURES.Minecraft
        ...
        local function applyWorldTexture()
            ...
            for _, obj in ipairs(game:GetDescendants()) do
                task.spawn(applyWorldTextureToObject, obj)
            end
        end
        ...
        game.DescendantAdded:Connect(function(obj)
            if worldTextureToggle and worldTextureToggle.value then
                task.defer(applyWorldTextureToObject, obj)
            end
        end)
