...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            do
                local cosmeticsToggle =
                    cosmetics:newToggle("misc/cosmetics/show_changer"):setLabel("Cosmetics")

                -- Connected cosmetic UI.
                -- The main Cosmetics toggle controls the visibility of the
                -- newly added cosmetic controls below.
                local cosmeticPanel = misc:newSection("right", "Skin Changer")
                local category = cosmeticPanel:newDropdown(
                    "misc/cosmetics/category",
                    false,
                    {"Skins", "Wraps", "Charm", "Finisher"}
                ):set("Skins")

                cosmeticPanel:newLabel("misc/cosmetics/status")
                    :setLabel("Select a cosmetic category")

                local function updateCosmeticPanel(enabled)
                    -- UISection has no public visibility method, so use its
                    -- container directly. This only affects the local UI.
                    if cosmeticPanel.instances and cosmeticPanel.instances.container then
                        cosmeticPanel.instances.container.Visible = enabled
                    end
                end

                cosmeticsToggle.changed:Connect(function(enabled)
                    updateCosmeticPanel(enabled)
                end)

                updateCosmeticPanel(cosmeticsToggle.value == true)

                category.changed:Connect(function(value)
                    local status = base.features["misc/cosmetics/status"]
                    if status then
                        status:setLabel("Selected category: " .. tostring(value))
                    end
                end)
            end
